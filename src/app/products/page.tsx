          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
              {products.map((product, index) => (
                <div key={product.id} className="rounded-xl overflow-hidden glass glow-card group animate-fade-slide-up flex flex-col"
                  style={{border:'1px solid rgba(0,123,255,0.15)', animationDelay: `${(index % 8) * 75}ms`}}>
                  <Link href={`/products/${product.id}`} className="block flex-1">
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="font-bold text-sm md:text-base text-slate-900 mb-2 line-clamp-2 group-hover:text-blue-600 transition" style={{fontFamily:'Montserrat,sans-serif'}}>
                        {product.name}
                      </h3>
                  ...
                  <div className="px-4 pb-4 pt-0 flex items-center gap-2">
                    <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleAddToCart(product); }} disabled={product.stock === 0}
                      className="flex-1 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-bold transition disabled:opacity-40 disabled:cursor-not-allowed"
